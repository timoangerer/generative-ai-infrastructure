...
def get_model_path_by_name(model_name, json_file='models.json', root_folder='models'):
    ...
    try:
        with open(json_file_path, 'r') as file:
            ...
            raise ModelNotFoundException(f"Model with name '{model_name}' not found.")
    except FileNotFoundError:
        raise JsonFileNotFoundException(f"File {json_file_path} not found.")

def get_model_path_by_hash(model_hash, json_file='models.json', root_folder='models'):
        ...
        with open(json_file_path, 'r') as file:
            ...
            for model in models:
                if model['hash'] == model_hash:
                    return os.path.abspath(os.path.join(root_folder, model['path']))
            raise ModelNotFoundException(f"Model with hash '{model_hash}' not found.")
    except FileNotFoundError:
        raise JsonFileNotFoundException(f"File {json_file_path} not found.")
